...
fn simd_u32(c: &mut Criterion) {
    let mut group = c.benchmark_group("SimdU32");

    let (cardinality, min_val, max_val) = (2, 0_u32, 20);
    ...
    for d in 0..=5 {
        ...
        let id = BenchmarkId::new("L2-simd", dimensionality);
        group.bench_with_input(id, &dimensionality, |b, _| {
            b.iter(|| black_box(simd::euclidean_u32(&vecs[0], &vecs[1])))
        });
        ...
        let id = BenchmarkId::new("L2-sq-simd", dimensionality);
        group.bench_with_input(id, &dimensionality, |b, _| {
            b.iter(|| black_box(simd::euclidean_sq_u32(&vecs[0], &vecs[1])))
        });
    }
    group.finish();
}

criterion_group!(benches, simd_f32, simd_f64, simd_u32);
